...
class DiskDriveVerifier
{
    ...
    private static ConsoleProgressBar? _progressBar = null;
    ...
    static void VerifyTestData()
    {
        ...
        try
        {
            ...
            foreach (var checksumEntry in savedChecksums)
            {
                ...
                if (!File.Exists(sourceFilePath))
                {
                    SafeWriteLine($"BŁĄD: Brakujący plik: {fileName}");
                    missingFiles++;
                    continue;
                }
                ...
                SafeWrite($"Kopiowanie i weryfikacja pliku {fileName} ({FormatBytes(fileInfo.Length)})... ");
                ...
                try
                {
                    ...
                    {
                        SafeWriteLine($"Weryfikacja {fileName}: OK ({speedMBps:F2} MB/s)");
                        verifiedFiles++;
                    }
                    else
                    {
                        SafeWriteLine($"Weryfikacja {fileName}: USZKODZONY");
                        corruptedFiles++;
                    }
                    ...
                    UpdateProgressBar(verifiedBytes, totalBytes);
                }
                catch (Exception ex)
                {
                    SafeWriteLine($"Błąd weryfikacji {fileName}: {ex.Message}");
                    corruptedFiles++;
        ...
        try
        {
            ...
            for (int i = 0; i < totalChunks; i++)
            {
                ...
                try
                {
                    ...
                    using (var destStream = new FileStream(targetChunkFile, FileMode.Create, FileAccess.Write, FileShare.None, 1024 * 1024, FileOptions.WriteThrough))
                    {
                        ...
                        while ((bytesRead = sourceStream.Read(buffer, 0, buffer.Length)) > 0)
                        {
                            ...
                            {
                                SafeWrite($"\rKopiowanie: [{new string('#', (int)(30 * copiedBytes / currentChunkSize))}{new string(' ', 30 - (int)(30 * copiedBytes / currentChunkSize))}] {(copiedBytes * 100.0 / currentChunkSize):F1}%");
                            }
                        }
                    ...
                    {
                        SafeWriteLine($"UWAGA: Plik {i+1} ma niepełny rozmiar: {FormatBytes(writtenFileInfo.Length)} zamiast {FormatBytes(currentChunkSize)}");
                    ...
                    SafeWriteLine($"Zakończono zapis pliku {i+1}/{totalChunks}: {FormatBytes(currentChunkSize)} w {chunkStopwatch.Elapsed.TotalSeconds:F1} s ({chunkSpeed:F2} MB/s)");
                    ...
                    UpdateProgressBar(writtenSize, testSize);
                ...
                catch (IOException ex)
                {
                    ...
                    {
                        SafeWriteLine("Brak wystarczającej ilości miejsca na dysku. Test zakończony.");
                        testFailed = true;
                    }
                }
            }
            ...
            if (failedChunks.Count > 0 && !testFailed)
            {
                ...
                // Rozpocznij nowy pasek postępu dla prób dodatkowych plików
                StartProgressBar("Uzupełnianie: ", 50);
                ...
                for (int attempt = 0; attempt < failedChunks.Count; attempt++)
                {
                    ...
                    while (newSize > 1024 * 1024) // Nie schodź poniżej 1 MB
                    {
                        ...
                        SafeWriteLine($"Próba zapisu dodatkowego pliku: {FormatBytes(newSize)}...");
                        ...
                        try
                        {
                            ...
                            using (var destStream = new FileStream(targetRetryFile, FileMode.Create, FileAccess.Write, FileShare.None, 1024 * 1024, FileOptions.WriteThrough))
                            {
                                ...
                                while ((bytesRead = sourceStream.Read(buffer, 0, buffer.Length)) > 0)
                                {
                                    ...
                                    // Aktualizuj pasek postępu dla bieżącego pliku
                                    SafeWrite($"\rKopiowanie: [{new string('#', (int)(30 * copiedBytes / newSize))}{new string(' ', 30 - (int)(30 * copiedBytes / newSize))}] {(copiedBytes * 100.0 / newSize):F1}%");
                                }
                            ...
                            {
                                SafeWriteLine($"UWAGA: Dodatkowy plik ma niepełny rozmiar: {FormatBytes(writtenFileInfo.Length)} zamiast {FormatBytes(newSize)}");
                            ...
                            SafeWriteLine($"Zapisano dodatkowy plik: {FormatBytes(newSize)} w {retryStopwatch.Elapsed.TotalSeconds:F1} s ({retrySpeed:F2} MB/s)");
                        ...
                        {
                            SafeWriteLine($"Nie udało się zapisać dodatkowego pliku: {ex.Message}");
                    ...
                    {
                        SafeWriteLine("Brak wystarczającej ilości miejsca na dysku. Przerwanie próby zapisu dodatkowych plików.");
                        break;
                    }
                }
    ...
    static void GenerateTestFile(string filePath, long size, int seed)
    {
        ...
        using (FileStream fs = new FileStream(filePath, FileMode.Create, FileAccess.Write, FileShare.None, 1024 * 1024, FileOptions.WriteThrough))
        {
            ...
            while (remainingBytes > 0)
            {
                ...
                {
                    UpdateProgressBar(totalWritten, size);
                }
            }
    ...
    static void DrawSimpleSpeedChart(List<SpeedMeasurement> measurements, string title, string unit)
    {
        ...
        for (int i = 199; i >= 0; i--) {
            if (topLine[i] != ' ' || bottomLine[i] != ' ') {
                maxLength = i + 1; // Poprawione: było 'l' zamiast '1'
                break;
            }
        }
    ...
    // Klasa do zarządzania paskiem postępu w konsoli
    class ConsoleProgressBar
    {
        private int _lastDrawnPercent = -1;
        private bool _isActive = false;
        private int _originalCursorTop = 0;
        private int _progressBarPosition = 0;
        private int _width;
        private string _prefix = string.Empty; // Inicjalizacja wartością domyślną
        private long _current;
        private long _total;
        private string _formatString = string.Empty; // Inicjalizacja wartością domyślną

        // Inicjalizacja paska postępu
        public void Initialize(int width = 50, string prefix = "Progress: ")
        {
            _width = width;
            _prefix = prefix;
            _current = 0;
            ...
            // Zapisz bieżącą pozycję kursora przed rozpoczęciem
            _originalCursorTop = Console.CursorTop;
            
            // Narysuj początkowy pusty pasek postępu
            UpdateProgressBar(0, 100);
            
            // Zapamiętaj pozycję paska postępu
            _progressBarPosition = Console.CursorTop;
            
            // Przesuń kursor z powrotem do oryginalnej pozycji, aby kontynuować wypisywanie
            Console.SetCursorPosition(0, _originalCursorTop);
        }

        // Aktualizacja wartości paska postępu
        public void Update(long current, long total, string formatString = "{0}/{1}")
        {
            if (!_isActive) return;
            ...
            // Odświeżaj tylko jeśli procent się zmienił (zmniejsza migotanie)
...
// Ukryj pasek postępu (tymczasowo lub na stałe)
        public void Hide()
        {
            if (!_isActive) return;
            
            // Zapisz bieżącą pozycję kursora
            int currentTop = Console.CursorTop;
            int currentLeft = Console.CursorLeft;
            
            // Przesuń kursor do pozycji paska postępu
            Console.SetCursorPosition(0, _progressBarPosition);
            
            // Wyczyść linię paska postępu
        ...
        public void Show()
        {
            ...
            _isActive = true;
            UpdateProgressBar(_current, _total);
        }

        // Zakończ pasek postępu i opcjonalnie dodaj nową linię
        public void Complete(bool addNewLine = true)
        {
            if (!_isActive) return;
            
            Update(_total, _total);
            ...
            if (addNewLine)
            {
                ...
                // Przesuń kursor za pasek postępu i dodaj nową linię
                Console.SetCursorPosition(0, _progressBarPosition + 1);
                
                // Zaktualizuj oryginalną pozycję kursora dla przyszłych danych wyjściowych
                _originalCursorTop = Console.CursorTop;
                
                // Przesuń kursor do nowej pozycji
                Console.SetCursorPosition(0, _originalCursorTop);
            }
        ...
        // Faktyczna metoda rysująca pasek postępu
        private void UpdateProgressBar(long current, long total)
        {
            try
            {
                ...
                for (int i = 0; i < _width; i++)
                {
                    ...
                    else if (i == completeWidth && i < _width)
                        progressBar += ">"; // Wskaźnik aktualnej pozycji
                    else
                        progressBar += "░"; // Niewypełniona część
                }
                progressBar += "]";
                
                // Sformatuj ciąg wyświetlania z wartościami current/total
                string bytesDisplay = string.Format(_formatString, 
                    FormatBytes(current), 
                    FormatBytes(total));
                
                // Pełny tekst do wyświetlenia
                string display = $"{_prefix}{progressBar} {percent:P1} {bytesDisplay}";
                
                // Sprawdź, czy pozycja paska postępu jest w granicach okna konsoli
                int windowHeight = Console.WindowHeight;
                
                // Pozycja paska postępu powinna być zawsze w granicach okna
                if (_progressBarPosition < 0 || _progressBarPosition >= windowHeight)
                {
                    // Jeśli poza granicami, ustaw na ostatnią widoczną linię
                    _progressBarPosition = windowHeight - 1;
                }
                
                // Przesuń kursor do pozycji paska postępu
                Console.SetCursorPosition(0, _progressBarPosition);
                
                // Wyczyść linię i narysuj pasek postępu
                Console.Write(new string(' ', Console.WindowWidth - 1));
                Console.SetCursorPosition(0, _progressBarPosition);
            ...
            {
                // Obchodzenie błędu pozycji kursora poza zakresem
                try
                {
                    // Spróbuj ustawić kursor na bezpiecznej pozycji
                    Console.SetCursorPosition(0, 0);
                    
                    // Wyłącz pasek postępu, aby uniknąć dalszych błędów
                    _isActive = false;
                }
                catch
                {
                    // Zignoruj błędy przy próbie odzyskania
                }
            }
            catch (Exception)
            {
                // Zignoruj inne błędy związane z konsolą
                _isActive = false;
            }
        }

        // Pomocnicza metoda do formatowania rozmiarów plików
        ...
        // Dodana metoda do sprawdzania, czy dana linia zawiera pasek postępu
        public bool IsProgressBarLine(int linePosition)
        {
            return _isActive && linePosition == _progressBarPosition;
        }
    }
    
    // Metody pomocnicze do obsługi paska postępu
    static void StartProgressBar(string prefix, int width = 50)
    {
        ...
        _progressBar.Initialize(width, prefix);
    }

    static void UpdateProgressBar(long current, long total)
    {
        if (_progressBar != null)
        {
            try
            {
                _progressBar.Update(current, total, "({0}/{1})");
            }
            catch (Exception)
            {
                // Zignoruj błędy przy aktualizacji paska postępu
                _progressBar = null;
            }
        }
    }

    static void CompleteProgressBar(bool addNewLine = true)
    {
        if (_progressBar != null)
        {
            try
            {
                _progressBar.Complete(addNewLine);
            }
            catch (Exception)
            {
                // Zignoruj błędy przy finalizacji paska postępu
            }
            _progressBar = null;
        }
    }

    // Bezpieczne metody wypisywania do konsoli z uwzględnieniem paska postępu
    static void SafeWriteLine(string message)
    {
        if (_progressBar != null)
        {
            try
            {
                ...
                // Wyczyść bieżącą linię TYLKO jeśli to nie jest linia z paskiem postępu
                if (!_progressBar.IsProgressBarLine(currentTop))
                {
                    ClearCurrentLine();
                }
                ...
                // Wymuś aktualizację paska postępu
                _progressBar.Show();
            }
            catch (Exception)
            {
                // W przypadku błędu, spróbuj normalnego wypisywania
                try { Console.WriteLine(message); } catch { }
                
                // Zresetuj pasek postępu, jeśli wystąpił błąd
                _progressBar = null;
            }
        }
        else
        {
            // Normalne wyjście, jeśli pasek postępu nie jest aktywny
            Console.WriteLine(message);
        }
    }
    ...
    static void SafeWrite(string message)
    {
        if (_progressBar != null)
        {
            try
            {
                ...
                // Wypisz komunikat bez nowej linii
    ...
    // Metoda do czyszczenia bieżącej linii w konsoli
    static void ClearCurrentLine()
    {
        ...
        {
            // Zignoruj błędy przy próbie czyszczenia linii
        }
    }
}